...
下文结构：第 \ref{sec:method} 节给出指数编制方法与同 S\&P 官方方法的差异；第 \ref{sec:data} 节说明数据来源、点位重建与数据质量检验；第 \ref{sec:results} 节报告回测结果；第 \ref{sec:portfolio} 节分析组合特征；第 \ref{sec:attribution} 节做收益来源分解；第 \ref{sec:cost} 节讨论交易成本；第 \ref{sec:hsmo} 节与 HSI300-Momentum-Strategy 比较；第 \ref{sec:robust} 节把两者的设计差异逐维度拆开，检验是否存在更好的折中；第 \ref{sec:conclusion} 节总结并逐条回答研究任务书中的 17 个问题。
...
\section{设计空间的稳健性与“折中”方案}\label{sec:robust}
第 \ref{sec:hsmo} 节的两套规则在多个维度上同时不同。本节把这些维度\textbf{逐项}拆开：从本指数出发，每次只改一个设计选择，其余全部保持不变（同一套点位数据、同一指数引擎、同一历史成本模型、同一公共区间、全收益费后口径），最后回答“中间地带是否存在更好的设计”。\textbf{本节是诊断性分析：已发布指数的规则不因此改变，配置文件中的参数也没有按结果调整。}
\FloatBarrier
\subsection{逐维度敏感性}
\begin{table}[!htbp]
\centering
\footnotesize
\caption{设计维度逐项变动（全收益、费后；前半段 = 2016 年之前，后半段 = 2016 年之后）}\label{tab:design}
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{}llrrrrrrrr@{}}
\toprule
维度 & 变体 & 年化（费后） & 波动 & 最大回撤 & 单边换手/年 & 成分数 & 有效成分数 & 前半段 & 后半段 \\
\midrule
基准 & A 本指数：top20\%，半年，FMC$\times$Score+上限，FV，缓冲，三周后实施 & 11.3\% & 26.9\% & $-$77.2\% & 108\% & 51 & 25 & 11.6\% & 11.9\% \\
成分数 & B1 成分数 top10\% & 10.4\% & 28.5\% & $-$79.8\% & 131\% & 26 & 18 & 9.3\% & 12.2\% \\
成分数 & B2 成分数 top5\% & 10.9\% & 30.1\% & $-$80.9\% & 147\% & 13 & 12 & 11.1\% & 11.8\% \\
成分数 & B3 成分数 固定20只 & 11.2\% & 29.0\% & $-$79.0\% & 133\% & 20 & 16 & 11.9\% & 11.5\% \\
频率 & C 频率 季度 & 10.9\% & 27.1\% & $-$76.0\% & 154\% & 51 & 25 & 10.7\% & 11.8\% \\
加权 & D1 加权 等权 & 11.5\% & 27.6\% & $-$78.2\% & 111\% & 51 & 51 & 14.6\% & 9.5\% \\
加权 & D2 加权 仅FMC & 11.3\% & 26.6\% & $-$76.8\% & 106\% & 51 & 26 & 11.8\% & 11.6\% \\
加权 & D3 加权 √FMC$\times$Score & 11.4\% & 27.1\% & $-$77.8\% & 109\% & 51 & 38 & 12.7\% & 11.0\% \\
筛选 & E 无财务资格筛选 & 10.9\% & 26.8\% & $-$77.3\% & 106\% & 58 & 28 & 10.9\% & 11.6\% \\
筛选 & F 无缓冲 & 11.4\% & 26.8\% & $-$76.9\% & 113\% & 51 & 25 & 11.4\% & 12.3\% \\
实施时点 & L 参考日收盘实施（无三周延迟） & 10.1\% & 27.0\% & $-$76.2\% & 108\% & 51 & 25 & 11.2\% & 9.8\% \\
折中 & G1 top10\%+季度 & 10.4\% & 28.9\% & $-$76.7\% & 187\% & 26 & 18 & 12.0\% & 9.7\% \\
折中 & G2 季度+参考日实施 & 10.7\% & 27.2\% & $-$73.7\% & 150\% & 51 & 25 & 12.0\% & 10.2\% \\
折中 & G3 top10\%+季度+参考日实施 & 10.9\% & 29.2\% & $-$73.8\% & 185\% & 26 & 18 & 15.0\% & 8.1\% \\
折中 & G4 top10\%+季度+参考日实施+√FMC$\times$Score & 10.7\% & 28.9\% & $-$73.8\% & 186\% & 26 & 21 & 14.8\% & 7.9\% \\
HSMO 端点 & H1 20只+季度+等权+无FV/缓冲 & 9.8\% & 29.1\% & $-$75.6\% & 223\% & 20 & 20 & 14.0\% & 6.8\% \\
HSMO 端点 & H2 H1 + 参考日实施 & 11.7\% & 29.4\% & $-$71.3\% & 225\% & 20 & 20 & 19.3\% & 5.7\% \\
\bottomrule
\end{tabular}
}
\end{table}

全部 17 个变体的费后年化收益落在 9.8\%--11.7\% 之间，基准（本指数）为 11.3\%。可以直接读出四条结论：(i) \textbf{更集中并不更好}——把成分数压到 10\%、5\% 或固定 20 只，收益不升反降，而波动、回撤和换手全部上升；(ii) \textbf{加权方案几乎不影响长期收益}——等权、纯市值、$\sqrt{\FMC}\times$Score 与本指数相差不到 0.1 个百分点，区别只在前后两段的分布（等权前半段更强、后半段更弱）；(iii) \textbf{财务资格筛选值得保留}——去掉后年化下降 0.48 个百分点，且前后两段一致；(iv) \textbf{把三周的实施延迟去掉、改在参考日收盘交易，单独看是有害的}（10.1\%），只有与季度调仓、高集中度组合在一起时才转为有利——这本身就说明这些差异不是稳定的结构性效应。
\FloatBarrier
\subsection{同一规则、不同调仓月份：设计差异小于日历噪声}
上述差异是否值得据以改变设计？一个直接的检验是：\textbf{保持规则完全不变，只把参考月份整体平移}。本指数用 2 月 / 8 月，这一选择来自 S\&P 的日程惯例，没有任何经济含义。把它换成其余 5 种半年相位，同样的规则给出的费后年化收益是 8.2\%--12.9\%（标准差 1.6 个百分点，极差 4.7 个百分点），\textbf{比上一小节全部设计变体之间的差距还要大}（图 \ref{fig:design} 左）。换言之，在 21 年样本上，1--2 个百分点的年化差异不足以区分两种设计的优劣。
这一结果同样适用于本文自己的指数：已发布的 2 月 / 8 月相位年化 11.3\%，而六个相位的平均为 10.6\%；相对沪深300全收益（9.7\%）的超额由 +1.6\% 降到 +0.9\%。也就是说，\textbf{已报告超额收益中约 0.8 个百分点来自调仓月份的运气}，而非规则本身。这一点在第 \ref{sec:results} 节的结论中必须一并考虑。
\FloatBarrier
\subsection{可以稳健改善的一件事：分批调仓}
\begin{table}[!htbp]
\centering
\footnotesize
\caption{把同一套规则拆成 $k$ 个错开调仓月份的子组合（每个子组合占 $1/k$ 资金，单位资金的换手率不变）}\label{tab:tranche}
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{}llrrrrrrrr@{}}
\toprule
分批数 & 可能组合数 & 年化均值 & 最低 & 最高 & 极差（pp） & 标准差（pp） & 波动 & Sharpe & 最大回撤 \\
\midrule
1 & 6 & 10.6\% & 8.2\% & 12.9\% & 4.7 & 1.48 & 27.0\% & 0.510 & $-$75.4\% \\
2 & 15 & 10.6\% & 8.9\% & 12.2\% & 3.3 & 0.94 & 26.7\% & 0.514 & $-$75.3\% \\
3 & 20 & 10.7\% & 9.5\% & 11.8\% & 2.3 & 0.66 & 26.7\% & 0.515 & $-$75.3\% \\
6 & 1 & 10.7\% & 10.7\% & 10.7\% & 0.0 & 0.00 & 26.6\% & 0.517 & $-$75.2\% \\
\bottomrule
\end{tabular}
}
\end{table}

既然“哪个月调仓”是纯粹的运气来源，就应当把它\textbf{分散掉}，而不是去挑一个幸运的月份：把资金分成 $k$ 份、各自遵循同样的规则但错开调仓月份（动量文献中的重叠组合，\citealp{jegadeesh1993}）。这是唯一一个\emph{事前}就能论证、无需回测支持的改进：单位资金的换手率不变，因而成本不变，但结果对日历选择的依赖被消除。$k=2$ 已经把极差从 4.7 个百分点压到 3.3，$k=3$ 压到 2.3，$k=6$（逐月错开）为零（图 \ref{fig:design} 右）；同时平均波动由 27.0\% 降至 26.6\%，平均 Sharpe 由 0.510 升至 0.517，平均最大回撤由 $-$75.4\% 改善到 $-$75.2\%。分批后的组合年化 10.7\%，相对沪深300全收益超额 +1.0\%——这是对该规则超额收益更诚实的估计。
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{../charts/research/rr_design_space.png}
\caption{左：逐维度设计变体的费后年化收益，灰带为同一规则在 6 种调仓相位下的区间；右：分批数与结果离散度}\label{fig:design}
\end{figure}

\FloatBarrier
\subsection{结论：折中方案是什么}
把上面的证据合起来，两套规则之间\textbf{沿“更集中 / 更高频”方向的中间点没有可靠的优势}——那一维度上的差异在日历噪声之内，而代价（波动、回撤、换手、容量）是确定的。真正值得采纳的折中是：
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
\item \textbf{保留本指数的宽度、财务资格筛选、缓冲与权重上限。}这些选择要么带来稳定的收益（筛选 0.48 个百分点），要么在收益相同的情况下降低波动、回撤与换手（宽度）。
\item \textbf{把调仓时点分成 2--3 批。}换手率与成本不变，消除掉一半到三分之二的日历运气，平均 Sharpe 与最大回撤同时小幅改善。这是本节唯一推荐的改动，且理由是事前的（分散一个无信息的选择），不依赖样本内表现。
\item \textbf{不要为 1--2 个百分点的样本内年化收益去调整成分数、频率或加权方案。}本节的证据表明这个量级低于噪声；按它调参得到的将是拟合结果，而不是改进。
\end{enumerate}
需要说明的是，分批实施改变的是\emph{组合的运作方式}而非指数定义：作为一只公开指数，本文的 CSI300\_SP\_FV\_SPMO 仍按 S\&P 的半年、提前公告的日程编制；分批属于跟踪该规则的\emph{产品}层面的实现选择。
\section{结论}\label{sec:conclusion}
把 S\&P 500 Momentum 的规则平移到沪深300并叠加 Financial Viability 盈利资格，在 2005--2026 年得到一条\textbf{长期期望为正、但需要忍受多年跑输}的规则：年化收益由 7.68\% 提高到 9.83\%（费后 9.33\%），Sharpe 由 0.43 升至 0.49，但波动与最大回撤更大，超额收益集中在 2017--2020 年，月度超额在统计上不显著。超额几乎全部来自选股而非 SPMO 式加权；本文的自由流通代理对国有大盘股的高配使报告的业绩偏保守。相同数据上，更集中、更高频的 Top-20 等权季度动量收益更高，但换手约两倍、集中度更高，其行业中性规则依赖当前分类而在历史上不可靠。第 \ref{sec:robust} 节进一步表明，两种设计之间的差距小于“在哪个月调仓”这一无信息选择造成的差异（同一规则、6 个相位：8.2\%--12.9\%）；已报告超额中约 0.8 个百分点应归于日历运气，而唯一可事前论证的改进是把调仓时点分成 2--3 批。研究任务书要求回答的 17 个问题逐条如下。
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
...
\item \textbf{Financial Viability + SPMO 是否显著改善沪深300的长期复利与风险调整收益}\quad 长期复利有实质改善（年化 7.68\% $\rightarrow$ 9.83\%，费后 9.33\%；21 年终值高 51\%），风险调整收益温和改善（Sharpe 0.43 $\rightarrow$ 0.49，Sortino 0.59 $\rightarrow$ 0.68，信息比率 0.20），但波动和最大回撤更大、超额收益集中在少数年份且统计上不显著；把调仓月份的相位差异计入后，超额的中枢估计还要再降约 0.8 个百分点（第 \ref{sec:robust} 节）。结论：\textbf{有改善，但不能称为“显著”}。
\end{enumerate}
