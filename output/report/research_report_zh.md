...
42 次已实施调仓共出现过 547 只不同的成分股。相邻两期成分股的平均保留率 45%；一段连续成员期的平均长度 1.8 期（中位数 1 期），51% 的成员期只持续一次调仓——动量组合的"短记忆"特征。入选次数最多的股票：贵州茅台（20 次）、五粮液（17 次）、恒瑞医药（16 次）、泸州老窖（15 次）、山东黄金（15 次）、格力电器（14 次）、三一重工（14 次）、同仁堂（14 次）、招商银行（14 次）、福耀玻璃（14 次）。
